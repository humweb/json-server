...
Please note that only array data type resources are supported`,
	...
	startCmd.Flags().BoolP("allow-all", "a", false, "Allow any Origin ie 'Access-Control-Allow-Origin: *' in the header")
...
func runStart(cmd *cobra.Command, _ []string) error {
	...
	api := &http.Server{
		Addr:    ":" + port,
		Handler: handler.Setup(resourceStorage, logs, allowAll),
...
func createResourceStorage(resourceKeys []string, filename string) (storage.Map, error) {
	resourceStorage := make(storage.Map)
